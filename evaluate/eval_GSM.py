...
def process_predictions(pred_file: str, output_file: str=None):
    ...
    for index, entry in enumerate(data):
        ...
        # matches = re.findall(r'\$\\boxed{(.*?)}\$', entry["pred"])
        pred = entry["generation"].split('\n\nQuestion:')[0]
        matches = re.findall(r'\$\\boxed{(.*?)}', pred)
        
        if len(matches) == 0:
            no_match_count += 1
            no_match_indices.append(f'{index}-{data[index]["generation"]}')
            pred_answer = None
        elif len(matches) == 1:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pred_file", type=str, default="/home/wangxin/projects/safety_steer/results/generations/gemma-2-9b-it/GSM/vanilla/outputs.json", help="Path to the input prediction file (JSON).")
