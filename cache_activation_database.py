            ...
            if hook_type == 'pre':
                ...
                def create_post_hook(cache):
                    def hook_fn(_, input_tensor, output_tensor):
                        # 对于 post-hook，Gemma的block输出是一个元组 (hidden_state, present_key_value)
...
def cache_response_activations(
    ...
    for query in tqdm(queries, desc="[1/5] 预计算和过滤数据"):
    ...
    final_shape = (total_tokens_to_cache, n_layers_cached, d_model)

    keys_memmaps = {
        hp: np.memmap(f"{foldername}/keys_{hp}.dat", dtype='float32', mode='w+', shape=final_shape)
        for hp in hook_points
    }
    ...
    with open(values_filepath, 'w') as f_values:
        for i in tqdm(range(0, num_valid, batch_size), desc="[2/5] 计算激活值"):
            ...
            input_ids_padded, activations_dict = get_activations(model_base, batch_full_text, layers, hook_points, seq_len)
            ...
            tqdm.write(f"[3/5] 精确提取并写入批次 {i//batch_size+1}...")
    ...
    # 5. 刷新 memmap 文件
    for memmap in tqdm(keys_memmaps.values(), desc="[4/5] 刷新数据文件到磁盘"):
        memmap.flush()

    # 6. 保存元数据文件
    print("[5/5] 保存元数据...")
    metadata = {
        "description": "Metadata for the cached activation database.",
        "source_script": "cache_activation_database.py",
        "shape": final_shape,
        "dtype": 'float32',
        "total_tokens_cached": total_tokens_to_cache,
        "num_layers_cached": n_layers_cached,
        "d_model": d_model,
        "hook_points_cached": hook_points,
    ...
    print(f"--- 缓存任务完成！元数据已保存至: {metadata_filepath} ---")
