...
def cache_activations(model_base: ModelBase, prompts: List[str], compute_activation_fn: Callable,
    ...
    for layer_idx, layer_num in enumerate(layers):
        ...
        memmap_files_acts[layer_idx] = ,(filepath, dtype='float32', mode='w+',
...
def main(args):
    ...
    def get_activations(model_base, prompts, substrings, seq_len, batch_size, tokens_to_cache, dataset_name, shard_size, label, cache_type):
        ...
        print('prompt:', prompts[0][0])
        print('substring:', substrings[0][0])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tokens_to_cache', type=str, default="<start_of_turn>model", help='How to find the position to cache. Options: "response", "entity", "last_eoi", "?", "random"')
...
#  CUDA_VISIBLE_DEVICES=2 python -m utils.activation_cache --model_alias gemma-2-2b --layers 13  --batch_size 4
#  CUDA_VISIBLE_DEVICES=6 python -m utils.activation_cache --model_alias gemma-2-2b --layers 13 --tokens_to_cache random --batch_size 32 --dataset pile